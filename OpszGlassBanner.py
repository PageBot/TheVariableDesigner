...
class AnimatedBannerFrame(AnimationFrame):
     ...
     def drawAnimatedFrame(self, view, origin):
            ...
            style['fontSize'] = self.vfFont.axes['opsz'][2]*self.magnifySizeFactor
...
for magnifySizeFactor, fontName in (
        (8, 'AmstelvarAlpha-VF'),
        (4, 'RobotoDelta_v2-VF')
    ):
    font = findFont(fontName)
    makeAnimation(font, W, H, c, magnifySizeFactor)
