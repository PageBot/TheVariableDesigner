...
class AnimatedBannerFrame(AnimationFrame):
     ...
     def drawAnimatedFrame(self, view, origin):
            ...
            instance = self.vfFont.getInstance({}) # Get neutral instance
            ...
            style = self.style.copy()
            instance = self.vfFont.getInstance(self.style['variableLocation'])
            style['font'] = instance.path
...
for axisTag in sequenceAxes:
    ...
    for axisFrameIndex in range(axisFrames):
        ...
        variableLocation = {axisTag: phisin*axisRange+minValue}
...
doc.export('images/%s_%s.gif' % (font.info.familyName, sample))
